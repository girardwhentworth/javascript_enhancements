...
(function () {
	...
	abP.controller('defCtrl', ['$scope', '$http', '$timeout', '$interval', '$q', '$sce', '$cookies', '$window', '$filter',
		function ($scope, $http, $timeout, $interval, $q, $sce, $cookies, $window, $filter) {
			...
			var beginExploringURL = '/beginexploring.html';
			var mvTime = 0;
			var adCompleted = 0;
			...
			var itemTimer = 0;
			var engageTimer = 0;
			// Fast forward timer for ngagement placements
			var maxEngagementWaitTimer = 0;
			var forwardNoTrackEngagements = false;
			...
			$http.get(window.promoServer + dataURL + window.key + '/200?v=1&tid=' + QueryString.tid + '&paper=' + QueryString.paper)
				.success(function (data) {
					...
					window.onbeforeunload = function () {
						if (window.placementIsComplete) return;
						var pingUrl = $scope.client.trackingUrl + 'f/t/closeSession?tid=' + QueryString.tid + '&paper=' + QueryString.paper + '&rock=' + QueryString.rock + "&encl=" + QueryString.encl;
                        var img = new Image();
                        img.src = pingUrl;
					}
					...
					} else if (!$scope.beginClicked && !$scope.client.autoStart && $('#content iframe').length) {
						$('#content iframe').attr('src', beginExploringURL);
					}
					...
					$('#content iframe').on('load', function () {
						var frameIsErrPage = false;
						var frameIsBeginPage = false;
						try {

							var frameLocPath = (typeof this.contentWindow.location != 'undefined' && typeof this.contentWindow.location.pathname != 'undefined') ? this.contentWindow.location.pathname : "";
							var errorPages = ['/errorPage', '/encrave_error.html'];
							frameIsErrPage = (errorPages.indexOf(frameLocPath) > -1);
							frameIsBeginPage = (frameLocPath == beginExploringURL);

							if (frameIsErrPage) {
								$scope.isError = true;
							} else if (!iframeLoaded[iframeIndex] && !frameIsBeginPage && !frameIsErrPage) {
							...
							if (!iframeLoaded[iframeIndex] && !frameIsBeginPage && !frameIsErrPage) {
								if ($scope.client.e != 0 && $scope.client.engageCountdown && engageTimer != 0 && $scope.client.disableReload == false) {
									$scope.clickingTooFast = true;
								} else if ($scope.client.disableReload == false || $scope.client.e == 0) {
									try {
                    ...
                    /*setInterval(function() {
                        ...
                        $scope.idleTimer++
                        if ($scope.idleWarned == false && $scope.idleTimer == $scope.client.timeout + (4 * 60)) {
                            $scope.idleWarned = true;
                            alert("Please continue onto the next link before your session becomes inactive");
                        }
                        if ($scope.idleWarned == true && $scope.idleTimer == $scope.client.timeout + (5 * 60)) {
                            alert("Your session is inactive, you will not be rewarded for this activity");
			...
			var clickEngaged = function (wasReloaded) {
				...
				// Start the engageMaxWaitTime timer
				if ($scope.client.e !== 0 && $scope.client.engageMaxWaitTime > 0) {
					maxEngagementWaitTimer = 0;
					forwardNoTrackEngagements = false;
					$interval.cancel(mxtmr);
					mxtmr = $interval(function () {
						maxEngagementWaitTimer++;
						if (maxEngagementWaitTimer == $scope.client.engageMaxWaitTime) {
				...
				if ($scope.client.engageCountdown && $scope.client.e != 0 && forwardNoTrackEngagements == false) {
			...
			eventer(messageEvent, function (e) {
				if (typeof e.data == 'string' && e.data.indexOf("encaction") == 0) {
					clickEngaged();
				}

				if (typeof e.data == 'string' && e.data.indexOf('encSendUserData') == 0) {
				...
				if (campaignType == 'serial') {
					for (var i = 0; i < $scope.client.displayLimit - minCount; ++i) {
						if ($scope.activeCampaigns.length < parseInt($scope.client.displayLimit)) {
							...
							if (typeof jspCampaign == 'object' && jspCampaign.type === 'search') {
								isSearch = true;
								break;
							}
						}
					...
					for (var ic = 0; ic < campaigns.length - minCount; ++ic) {
						if ($scope.activeCampaigns.length < parseInt($scope.client.displayLimit)) {
							...
							if (typeof campaign == 'object' && campaign.type == 'jsonp') {
								isJSONP = true;
							...
							if (typeof campaign == 'object' && isSearch === true) {
								...
								if (typeof campaign == 'object' && campaign.type === 'links') {
			...
			$scope.viClick = function (clickType) {
				...
				$timeout(function () {
					if (!iframeLoaded[$scope.tempIframeIndex] && $scope.isError == false && ($scope.client.disableReload == false || $scope.client.e == 0)) {
						clickEngaged(true);
					}
				}, 5 * 1000);
			};
			...
			$scope.adIframe = function (ad, index) {
				...
				if (itemTimer === 0) {
					var tos = typeof ad.tos !== 'undefined' ? ad.tos : $scope.client.timeout;
					itemTimer = parseInt(tos);
				}
				...
				$http.jsonp(adLink + '&callback=JSON_CALLBACK')
					.success(function (d) {
						...
						if (d.success == true) {
							...
							if(typeof ad.redirectUrl !== 'undefined') {
							    d.redirectUrl = ad.redirectUrl;
							}
			...
			var validIframeClick = function () {
				...
				var cdTmr = $interval(function () {
					...
					// if the engagement max wait timer has completed
					if ($scope.client.e !== 0 &&
						$scope.client.engageMaxWaitTime > 0 &&
						maxEngagementWaitTimer == $scope.client.engageMaxWaitTime) {
						itemTimer = 0;
						engageTimer = 0; // reset to prevent quick-clicking errors
						maxEngagementWaitTimer = 0;
						forwardNoTrackEngagements = true;
						$scope.engagementCountdown = false;
						$scope.task.current = $scope.task.total;
					}
					if (itemTimer === 0) {
						// prevent the timer from incrementing if we're fast forwarding
						if ($scope.client.e !== 0 && $scope.task.total !== 0 && $scope.task.total !== $scope.task.current && forwardNoTrackEngagements == false) {
							++itemTimer;
						} else {
							// prevent the engagement firing if we're fast forwarding
							if ($scope.task.total === $scope.task.current && $scope.client.e !== 0 && parseInt(ad.serverPostBack) !== 1 && forwardNoTrackEngagements == false) {
			...
			var completedPromo = function () {
				$timeout(function () {
					...
					if (typeof QueryString.tid !== 'undefined' &&
						...
						waitForJSONP = true;
						var pixelURL = pixelServer + 'f/t/pixel?tid=' + QueryString.tid + '&paper=' + QueryString.paper + '&rock=' + QueryString.rock + "&encl=" + QueryString.encl + '&urrMet=' + QueryString.urrMet;

						$http.jsonp(pixelURL + '&callback=JSON_CALLBACK')
							.success(function (d) {
								if (d.success == true) {
									window.placementIsComplete = true;
									$('#complete').fadeIn();
									...
									if (typeof window.isModal != 'undefined' && window.isModal == true && typeof QueryString.isPub == 'undefined') {
										...
										if (keepCravingAuto && $scope.verifyCompleted.URL != "") {
									...
									var errorMessages = {
										...
										BrowserSize: 'Your browser is currently sized too small for this offering.'
