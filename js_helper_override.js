/*
	Edit-004 - Trying to force $scope.client.autoPlaylist to be true
	Edit-005 - Trying to force $scope.client.startedAutoPlaylist  to be true
	Edit-006 - Commented out last two commits, trying to call "dmClick"
	Edit-007 - Reverting last commit
	Edit-008 - Reverted 002, 003, 004, 005, 006. Added test function to click the button instead
	Edit-009 - Added pre-link alert to check if it's firing
	Edit-010 - It did find link, but too soon to click
	Edit-011 - This worked! Commit to remove alerts. Slight increase to minimum timeout (countdown will be between 9 and 17 seconds)
	Edit-012 - Adding auto-close of the tab when the path is completed
	Edit-013 - May not need to always click the ".switch" link. Let's check the class value first.
	Edit-014 - Sync with manual github. Consider ad campaign success after one video
	Edit-015 - Force success and remove error messages.
	Edit-016 - Revert change 014 and consider ad campaign success after 1/4 the videos
	Edit-017 - Revert change 016 and consider complete after 4 videos
...
(function () {
	...
	abP.controller('defCtrl', ['$scope', '$http', '$timeout', '$interval', '$q', '$sce', '$cookies', '$window', '$filter',
		function ($scope, $http, $timeout, $interval, $q, $sce, $cookies, $window, $filter) {
			...
			// Edit-003
			// alert('In variable land!');
			
			// Edit-004
			// $scope.client.autoPlaylist = true;
			// Edit-005
			// $scope.client.startedAutoPlaylist = true;
			...
			$http.get(dataURL + window.key + '/200?v=1')
			    ...
			    csiframe.src = 'javascript:window["contents"]';
					...
					// Edit-006
					// $scope.client.autoPlaylist = true;
					// dmClick(); // this doesn't work
					
					$scope.client.autoPlaylist = true;
					
					// Edit-002
					// alert('$scope.client.autoPlaylist = ' + $scope.client.autoPlaylist + '\n$scope.client.startedAutoPlaylist = ' + $scope.client.startedAutoPlaylist);
					...
					setTimeout(function() {
						// Edit-009					
						// alert('Pre-link alert');
						var link = document.querySelector('.switch');
						// Edit-013 - if the switch is already active (class = "switch active"), then don't click it. Will be "switch" if inactive.
			...
			var completedItem = function () {
				...
				$scope.pe = -1;
				//Edit-014
				// adCompleted = $scope.activeCampaigns.length;
									...
									.success(function (d) {
										...
										//if (d.success == true) {
											...
											if (typeof window.isModal != 'undefined' && window.isModal == true && typeof QueryString.isPub == 'undefined') {
												...
												if(keepCravingAuto){
													//Edit-001
													alert('keepCravingAuto = ' + keepCravingAuto);
			...
			var showCompleted = function () {
				// Edit-001
				// alert('$scope.guideMod = ' + $scope.guideMod);
